...
doc=etree.HTML(html)
region=doc.xpath('//*[@id="main"]/div[2]/div/div/div/div/div[1]/table/tbody/tr/td[1]/span/text()')
...
import pandas as pd
df=pd.DataFrame({'regions':region,'infected':infected,'dead':dead})
df2=df.sort_values(by='infected',ascending=False).iloc[0:15]
import matplotlib
matplotlib.rcParams['font.family']='simHei'
import matplotlib.pyplot as plt
x,y=df2['regions'].map(lambda x:'\n'.join(x)),df2['infected']
